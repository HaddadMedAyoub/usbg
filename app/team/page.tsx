...
// 4‑1‑3‑2 slots (unchanged)
const lineupSlots = [
  { id: "ls", top: "12%", left: "33%" },
  { id: "rs", top: "12%", left: "67%" },
  { id: "lm", top: "33%", left: "22%" },
  { id: "cm", top: "31%", left: "50%" },
  { id: "rm", top: "33%", left: "78%" },
  { id: "ax", top: "52%", left: "50%" },
  { id: "lb", top: "68%", left: "15%" },
  { id: "lcb", top: "68%", left: "37%" },
  { id: "rcb", top: "68%", left: "63%" },
  { id: "rb", top: "68%", left: "85%" },
  { id: "gk", top: "86%", left: "50%" },
];

// slugs must match Supabase rows
const LINEUP: Record<string, string> = {
  gk: "gazzeh-raed",
  lb: "yeken-mohamed-habib",
  lcb: "touis-iyed",
  rcb: "daniel-oboh",
  rb: "abderrazzak-ghazi",
  ax: "wael-salehi",
  lm: "bida-junior",
  cm: "maaouani-khemais",
  rm: "ben-mcharek-ayoub",
  ls: "idris-muhairssi",
  rs: "lamine-touray",
};
...
function PlayerAvatar({ player, size = "md" }: { player: Player; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div
      className={`${sizes[size]} rounded-full ring-2 ${positionRing[player.position]
        } shrink-0 flex items-center justify-center font-black text-white ${positionDot[player.position]
        } ${textSizes[size]}`}
    >
      {player.number ?? "?"}
    </div>
...
export default function TeamPage() {
  ...
  const lineupPlayers = useMemo(() => {
    ...
    for (const [slotId, slug] of Object.entries(LINEUP)) {
      map[slotId] = players.find((p) => p.slug === slug) ?? null;
    }
    return map;
  }, [players]);

  const totalGoals = useMemo(
    () => players.reduce((s, p) => s + (p.goals || 0), 0),
    [players]
  );
    ...
    <div className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="relative px-4 pt-12 pb-10 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <h1 className="text-white font-black text-4xl sm:text-5xl mb-2">
            التشكيلة
          </h1>
          ...
          <div className="flex gap-6 mt-6">
            {[
              { label: "لاعب", value: players.length },
              { label: "هدف", value: totalGoals },
              {
                label: "دقيقة",
                value: totalMinutes.toLocaleString("ar-TN"),
              },
            ].map((stat) => (
              <div key={stat.label}>
      ...
      <div className="px-4 max-w-4xl mx-auto mb-14">
        ...
        <div
          className="relative w-full rounded-2xl border border-[#1f1f1f]"
        ...
        >
          <div
            ...
            style={{
              background:
                "radial-gradient(ellipse at 50% 50%, #0d3d0d 0%, #071a07 60%, #040f04 100%)",
            }}
          >
            <svg
            ...
            return (
              ...
              >
                ...
                <span className="text-white text-[9px] font-bold bg-black/70 px-1.5 py-0.5 rounded-full whitespace-nowrap backdrop-blur-sm max-w-[60px] truncate">
                  {player.name_ar.split(" ").slice(-1)[0]}
                </span>
              </button>
            );
          })}
      ...
      {selected && (
        ...
        >
          <div
            className={`bg-[#0f0f0f] border rounded-3xl p-6 w-full max-w-sm bg-gradient-to-b ${positionGradient[selected.position]
              } shadow-2xl`}
          ...
          >
            <div className="flex items-center gap-4 mb-5">
              ...
              <div className="flex-1">
                <span
                  className={`text-[10px] font-bold px-2 py-1 rounded-lg border ${positionBadge[selected.position]}`}
                >
            ...
            <div className="grid grid-cols-3 gap-3 mb-4">
              {[
                ...
                >
                  <p className="text-[#F7C600] font-black text-xl">
                    {s.value}
                  </p>
            ...
            <div className="flex gap-3">
              ...
              <a
                href={`/team/player?slug=${selected.slug}`}
  ...
  return (
    <div className="flex flex-col gap-2">
      {players.map((player) => (
        ...
        >
          <div
            className={`w-10 h-10 rounded-full ring-2 ${positionRing[player.position]} shrink-0 flex items-center justify-center font-black text-white text-xs ${positionDot[player.position]}`}
          >
            {player.number ?? "?"}
          </div>
          ...
          <span
            className={`text-[10px] font-bold px-2 py-1 rounded-lg border shrink-0 ${positionBadge[player.position]}`}
          >
          ...
          <span className="text-gray-700 text-xs group-hover:text-[#F7C600] transition-colors shrink-0">
            ←
          </span>
        </button>
      ))}
